...
import { ICanFindPackagesByUser, packageIsToolingPackage } from './index';
...
export class NpmPackageByUserFinder implements ICanFindPackagesByUser {
    ...
    async find(user: string, check: (packageJson: any) => boolean = (_) => true) {
        let packages = await npmUserPackages(user);
        return packages.filter(packageJson => packageIsToolingPackage(packageJson) && check(packageJson));
    }
}