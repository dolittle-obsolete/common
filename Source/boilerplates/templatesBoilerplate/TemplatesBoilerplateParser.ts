...
export class TemplatesBoilerplateParser implements ICanParseBoilerplates {
    ...
    async parse(boilerplate: any, boilerplatePath: string) {
        ...
        const contentDirectory = templatesBoilerplateContentDirectoryFromPath(boilerplatePath);
    ...
    private async createTemplates(contentDirectory: string) {
        const templateFiles = await this._folders.getFilesRecursively(contentDirectory, new RegExp(templateConfigurationName));
        const templates: ITemplate[] = [];
        await Promise.all(templateFiles.map(async templateFile => {
            const includedFiles = await this.getIncludedFiles(getFileDirPath(templateFile));
            const templateJson = await this._fileSystem.readJson(templateFile);
            const template = templateFromJson(templateJson, templateFile, includedFiles, this._dependencyParsers);
    ...
    private async getIncludedFiles(folderPath: string) {
        const filesInFolder = await this._folders.getFiles(folderPath, /.*/);
        const filesExceptTemplateConfig = filesInFolder.map(filePath => getFileNameAndExtension(filePath)).filter(file => file !== templateConfigurationName);
