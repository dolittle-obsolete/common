...
export class LocalPluginsDiscoverer implements ICanDiscoverPlugins {
    ...
    async discover(folder?: string) {
        ...
        this._pluginPaths = [];
        const discoveredPluginPackages = await this._localPackageDiscoverers.discover(folder, _ => packageIsPluginPackage(_));

        const pluginsConfigObject: {[pluginPackageName: string]: PluginConfigObject} = {};

        for (const discoveredPlugin of discoveredPluginPackages) {
            const folderPath = discoveredPlugin.path;
            const toolingPackage = discoveredPlugin.package;
            const pluginJavascriptFiles = [
                path.join(folderPath, 'Distribution', 'index.js'),
            ];
            let foundPlugin = false;
            for (const pluginPath of pluginJavascriptFiles) {
                if ((await this._fileSystem.exists(pluginPath))) {
                    ...
                    if (packageIsCompatible(toolingPackage, this._toolingPackage)) {
                        ...
                        const pluginPackage = new PluginPackage(toolingPackage, pluginPath);
