...
describe('and there are only valid dependencies', () => {
    let first_dependency = new DiscoverDependency('first dependency', 'desc', [], namespaceDiscoverType,  undefined, 'some milestone');
    let second_dependency =  new DiscoverDependency('second dependency', 'desc', [], namespaceDiscoverType,  undefined, 'some milestone');
    let dependencies = [
        first_dependency,
        second_dependency
    ];
    let result = new Dependencies(dependencies, new StandardValidatorsForDependency());
