 ...
 *--------------------------------------------------------------------------------------------*/
import { IDependencyResolvers, ICanResolveDependencies, IDependency, MultipleResolversForDependency, CannotResolveDependency, IDependencyRuleFor  } from '../index';
import { ILoggers } from '@dolittle/tooling.common.logging';
 ...
 export class DependencyResolvers implements IDependencyResolvers {
    ...
    async resolve(context: any, dependencies: IDependency[], additionalRules: IDependencyRuleFor<IDependency>[], destinationPath?: string, coreLanguage?: string, args?: string[], options?: Map<string, any>): Promise<any> {
        ...
        for (let entry of resolversMap) {
            ...
            let deps = entry[1];
            context = await Promise.resolve(resolver.resolve(context, deps, additionalRules, destinationPath, coreLanguage, args, options));
        }
